...
**Date:** 2026-08-02
**Validated by:** qa (`tests/validation.test.js`)
...
**Result: PASS** — `tests/validation.test.js:75`
...
**Result: PASS** — `tests/validation.test.js:136`
...
**Result: PASS** — `tests/validation.test.js:214`
...
(`tests/reconcile.test.js`, 20 tests), e2e tests (`tests/reconcile-cli.test.js`, 5 scenarios),
...
node --test 'tests/reconcile-cli.test.js'       # 5 scenarios, all pass
...
- **Primary path would also work, with caveat.** The global `armada` binary is not installed
  on this machine. The `tests/armada-resume-command.test.js` test proves the primary path
...
- **Native agents.** 8 `.opencode/agent/*.md` files generated with valid frontmatter
  (mode/model/permission). Listing + frontmatter parse verified.
...
- **Manifest round-trip.** `parseManifestYaml(evidence-target/armada/armada.yaml)` accepts
  it (8 team members, correct stack).
- **Stack detection (live target).** The live `~/WBG/data-ai-chatbot` re-scaffold is a
  user-side manual step (walkthrough below); the in-lane validation runs against the
  in-sandbox `evidence-target/` facsimile. The lane can read the live target's files
  (read-only, no writes): inspection of `opencode.json`, `armada/armada.yaml`, and
  `.opencode/agent/*.md` confirmed stack detection matches.
...
- **Manual walkthrough.** Exact commands for the user to run in `~/WBG/data-ai-chatbot`:
  ...
  resume → cleanup. Each step has expected output + a "what to check" line (full text below).

### Live target status

`~/WBG/data-ai-chatbot` was scaffolded with an older armada (omo-slim era). Re-scaffold
is a user-side manual step; commands in the walkthrough below. The clone-based validation
above confirms the generated repo path works end-to-end; the live target needs the
re-scaffold step before the walkthrough.

### Manual Resume Walkthrough for `~/WBG/data-ai-chatbot`

This walkthrough proves the restart-proof resume path works in the live target repo. The user
runs these commands interactively.
...
Check: `armada/state/active.json` exists with `"feature": "scratch-resume-walkthrough"` and
phase-1 = "pending".
...
```
Expected: phase-1 status is "pending". Copy the structure from `armada/REQUIREMENTS.md` in
this sandbox — `phaseGraph.phases[].id|title|status|criteria[].id|text|evidence` is the
schema. A minimum one-phase contract is enough.
...
The orchestrator reads `armada/state/active.json`, finds phase-1 pending, updates it to
`in_progress`, and starts working.
...
If `armada` binary is not on PATH, the fallback `node src/cli.js reconcile` won't work in a
generated repo (no src/). Install the global binary.
