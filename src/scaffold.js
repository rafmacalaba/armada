...
export function scaffold(manifest, stack, opts = {}) {
  ...
  // 1d. Orphan cleanup: remove slash-command files retired in v0.9.0+.
  ...
  //    unless force is true (--restart) which bypasses the guard.
  if (opts.force || !existsSync(out("opencode.json"))) {
  ...
  // 7. armada commands for in-session use — exactly 4.
