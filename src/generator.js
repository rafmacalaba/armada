...
export function renderAgentsMd(manifest, team) {
  ...
  return `# ${manifest.project.name} — Build Rules

These rules apply to every agent working on this project. Generated by opencode-armada.

## The job

Build exactly as specified in [REQUIREMENTS.md](./REQUIREMENTS.md). That document is the
...
Each phase in REQUIREMENTS.md ends with a gate: every success criterion must be demonstrated
by evidence — a passing test run, a screenshot, or both. Only then does the next phase start.
`
}

// Build `REQUIREMENTS.md` — the contract scaffold. Only written when absent.
export function renderRequirementsMd(manifest) {
  return `# ${manifest.project.name} — Requirements

> The contract. Generated by opencode-armada. Fill in phases, success criteria, and final
> criteria. When in doubt, this document wins.
...
# Regenerate identical config with: armada init --from-armada armada.yaml
