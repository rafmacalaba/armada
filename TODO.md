...
- [x] **feat(examples): embed blank-webpage demo** (#122) (2026-08-06). Copy `~/WBG/my-app` verbatim into `examples/blank-webpage/` as a worked example of an armada-driven project (static-landing starter + armada scaffold). Adds a 61-line README covering identity, how to read process artifacts, source provenance, and reference-only scope. 11 source files tracked (HTML/CSS/JS/README/AGENTS/TODO/.gitignore/.gitkeep); `.opencode/`, `opencode.json`, `armada/` scaffold intentionally untracked via root + embedded `.gitignore` so the parent repo's fleet config stays canonical. 446/446 parent tests green.
