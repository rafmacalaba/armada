...
- End-to-end tests live under `armada/e2e/pr-first/`. Only qa writes there.
- Screenshots live under `armada/screenshots/pr-first/`.
...
## armada/ledgers/pr-first/DEFECTS.md — the defect ledger

All defects live in `armada/ledgers/pr-first/DEFECTS.md`, one entry per defect, newest
...
Format, exactly:
    ...
    Screenshot: armada/screenshots/pr-first/def-001.png (optional)
...
## armada/ledgers/pr-first/ADVERSARIAL_REVIEW.md — the adversary's findings

All adversary findings live in `armada/ledgers/pr-first/ADVERSARIAL_REVIEW.md`. Writers: **adversary** (create
...
Format, exactly:
    ...
    Expected: ...
    Actual: ...
    Screenshot: armada/screenshots/pr-first/adv-001.png (optional)

    Disposition: PENDING

The orchestrator replaces PENDING with either `ACCEPTED -> DEF-NNN` or `REJECTED - reason`.
Accepted findings are reproduced and filed in armada/ledgers/pr-first/DEFECTS.md by qa. No entry may remain PENDING
