...
armada generates reproducible AI-engineer multi-agent teams for opencode, natively —
...
- Scaffold a team into a repo: `node src/cli.js init --from-armada armada/armada.yaml`
...
- Opt-in supervision: `armada init --supervision-plugin` (or `armada.yaml`
  ...
  `--watchdog` for `armada-watchdog.js` — nudges the orchestrator when a dispatched subagent
  has been pending past `TIMEOUT_MS` (5 min) AND the orchestrator has been idle for
  `STALENESS_WINDOW_MS` (2 min).
...
# orchestrator-no-trivial — Build Rules
...
- **orchestrator** — Delivery lead / scheduler. Model `openrouter/anthropic/claude-sonnet-4.6`.
- **backend-dev** — Backend implementation. Model `openrouter/z-ai/glm-5.2`.
- **frontend-dev** — Frontend implementation. Model `openrouter/minimax/minimax-m3`.
- **qa** — Quality assurance. Model `openrouter/xiaomi/mimo-v2.5`.
- **adversary** — Adversarial reviewer. Model `openrouter/deepseek/deepseek-v4-pro`.
- **security** — Security auditor. Model `openrouter/deepseek/deepseek-v4-pro`.
- **docs** — Technical writer. Model `openrouter/minimax/minimax-m3`.
- **architect** — Architecture / code review. Model `openrouter/z-ai/glm-5.2`.
